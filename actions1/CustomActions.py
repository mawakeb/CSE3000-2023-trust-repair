...
class RemoveObjectTogether(Action):
    ...
    def mutate(self, grid_world, agent_id, world_state, **kwargs):
        ...
        for obj in objects_in_range:  # loop through all objects in range
            ...
            if condition!='baseline' and obj == object_id and get_distance(other_agent['location'], world_state[obj]['location'])<=remove_range and get_distance(other_human['location'], world_state[obj]['location'])<=remove_range and \
                (('tree' in obj and condition != 'mixed') or ('rock' in obj or 'stone' in obj)):
