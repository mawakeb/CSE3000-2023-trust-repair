...
def output_logger(fld):
    ...
    with open(glob.glob(os.path.join(fld, 'data.csv'))[0]) as csvfile:
        reader = csv.reader(csvfile, delimiter=';', quotechar="'")
        for row in reader:
            if data_header == []:
                data_header = row
            ...
            if row == []:
                continue
            res = {data_header[i]: row[i] for i in range(len(data_header))}
            data_contents.append(res)

    arrival_wait_time_1 = data_contents[-1]['arrival_wait_time_1']
    arrival_wait_time_2 = data_contents[-1]['arrival_wait_time_2']
    arrival_wait_time_3 = data_contents[-1]['arrival_wait_time_3']
    arrival_wait_time_4 = data_contents[-1]['arrival_wait_time_4']
    message_wait_time_1 = data_contents[-1]['message_wait_time_1']
    message_wait_time_2 = data_contents[-1]['message_wait_time_2']
    message_wait_time_3 = data_contents[-1]['message_wait_time_3']
    message_wait_time_4 = data_contents[-1]['message_wait_time_4']
